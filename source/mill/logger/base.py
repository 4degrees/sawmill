...
class Logger(Log):
    ...
    emit :py:class:`~mill.log.Log` records with that information already
    present.

    '''

    def __init__(self, _handler=mill.root, **kw):
        '''Initialise logger.
    ...
    def prepare(self, *args, **kw):
        ...
        *kw* arguments are automatically mixed in to a
        :py:class:`~mill.log.Log` record made by copying this current logger.
    ...
    def log(self, *args, **kw):
        '''Emit a :py:class:`~mill.log.Log` record.'''
