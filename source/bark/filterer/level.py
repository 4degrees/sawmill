...
class Level(Filterer):
    ...
    def __init__(self, min='info', max=None):
        '''Initialise filterer with *min* and *max* levels.

        The values must be taken from the configured :py:attr:`bark.levels`
        array. If the value given is None then it will be as if there
        is not limit.
    ...
    def filter(self, log):
        ...
        try:
            level_index = bark.levels.index(level)
        except ValueError:
            return False

        # Check against defined range.
        if self.min is not None:
            min_index = bark.levels.index(self.min)
            if level_index < min_index:
                return True

        if self.max is not None:
            max_index = bark.levels.index(self.max)
