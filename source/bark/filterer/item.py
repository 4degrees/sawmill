...
class Item(Filterer):
    ...
    def __init__(self, key, value, mode=INCLUDE):
        ...
        *mode* can be either :py:attr:`~bark.filterer.item.Item.EXCLUDE` or
        :py:attr:`~bark.filterer.item.Item.INCLUDE`. If set to
        :py:attr:`~bark.filterer.item.Item.EXCLUDE` then any log that has the
        specific *key*, *value* pair will be filtered. Conversely, if set to
        :py:attr:`~bark.filterer.item.Item.INCLUDE` then any log not matching
